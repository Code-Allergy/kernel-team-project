 ...
 * We can decide on where we place the user heap and user stack in virtual
 * memory.
 */
...
/* For now identity map the entire physical memory here, so we can access it
 * with ttbr0 page. */
...
/* This maps directly to the start of physical memory, but will let us enforce
 * page permissions and be free to move the kernel around in physical memory if
 * required. This is where our linker script will place the kernel, make sure
 * sections are 4k aligned. We probably also want a header section of the
 * kernel, that will contain some basic info.
 */

/* Hardware and device mapping region (256MB) */
#define MEM_DEVICE_BASE 0xC0000000 /* 0xC0000000-0xCFFFFFFF */
/* Remap physical addresses from lower memory to here, so we can access with
 * ttbr1 page. This region needs to specifically be uncached. We can also slice
 * up some of this extra space for DMA buffers. Might also remap our other
 * memory here, if we want to access it.
 */
...
/*
 * The regions are larger than we will ever need, but it makes it easier to
 * understand the type of pointer we are working with.
 */
