...
static const uint32_t timer_base[8] = {-1,
                                       -1,
                                       TIMER2_BASE,
                                       TIMER3_BASE,
                                       TIMER4_BASE,
...
static const uint32_t timer_module_clk_off[8] = {-1,
                                                 -1,
...
static const uint32_t timer_pll_clksel_off[8] = {-1,
                                                 -1,
...
static const int16_t timer_irq_num[8] = {130,
                                         130,
                                         TINT2_INT_NUM,
                                         TINT3_INT_NUM,
                                         TINT4_INT_NUM,
...
static const void_func timer_irq_handlers[8] = {NULL,
                                                NULL,
                                                handle_timer2_irq,
                                                handle_timer3_irq,
                                                handle_timer4_irq,
                ...
                void (*timer_tick_func)(void))
{
    ...
    while ((REG32_read(CM_DPLL_BASE, timer_pll_clksel_off[timer_index]) &
            0x3) != 0x2)
        ;
    /*Clock enable*/
    REG32_write_masked(
        CM_PER_BASE, timer_module_clk_off[timer_index], 0x3, 0x2);
    while ((REG32_read_masked(
                CM_PER_BASE, timer_module_clk_off[timer_index], 0x3 << 16) &
            0x3))
        ;
    ...
    while (
        (REG32_read_masked(timer_base[timer_index], TIOCP_CFG_OFF, 0x1) & 0x1))
        ;
    ...
    /*Timer control setup*/
    REG32_write_masked(timer_base[timer_index],
                       TCLR_OFF,
                       0x7FFE,
                       0x0000 | (0x1 << 1) /*AR*/
                           | (0x0 << 2)    /*PTV*/
    ...
    REG32_write(timer_base[timer_index],
                TLDR_OFF,
    ...
    INTC_register_irq(timer_irq_num[timer_index],
                      timer_irq_handlers[timer_index]);
