...
typedef struct {
    ...
    // int (*read_sectors)(uint32_t sector, uint8_t* buffer, uint32_t count); -- todo DMA

    // int (*write_sector)(uint32_t sector, uint8_t *buffer); --
    // todo, writes don't break the FAT but corrupt the FSINFO sector, by not updating the free cluster count
} fat32_diskio_t;
