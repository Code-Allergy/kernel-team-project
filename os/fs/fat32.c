...
static void initialize_fat32_fs(fat32_fs_t *fs, const fat32_diskio_t *io,
...
int fat32_read(fat32_file_t *file, void *buffer, int size) {
    ...
    while (remaining > 0) {
        ...
        if (cluster_result < 0) return cluster_result;
...
static void initialize_fat32_fs(fat32_fs_t *fs, const fat32_diskio_t *io,
    ...
    fs->total_sectors = (boot_sector->totalSectors16 != 0)
                        ? boot_sector->totalSectors16
                        : boot_sector->totalSectors32;
    fs->total_clusters = (fs->total_sectors - (fs->first_data_sector - fat32_start_sector))
